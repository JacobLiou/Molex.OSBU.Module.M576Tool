...
2. **`RECAL 0`（配置扫参）**：设置 TLS 光源通道、delay、DAC range、DAC step；在一次批量校准开始前发送，或按固件要求在变更参数时重复发送。
3. **循环体（1286 步或路径表行数）**：对每一步发送 **`RECAL 1` 或 `RECAL 2`**（见下节），接收固件返回的**功率值序列**。
...
| type | 含义 | 典型参数 |
|------|------|----------|
| **0** | 配置 TLS 与扫参 | TLS source 1–8；delay（范围与单位见固件）；DAC range 1–200；DAC step 1–100 |
...
- `path_0330` 正式导出 CSV 及变更记录（若路径表有修订）。
