...
BOOL CPDBoxForMCS::NewGetMaxPdCout(int chan, int npoint, int *maxid, double *maxpow)
{
	int errcout;
	errcout = 0;
GETMAX:	CString strCommand;
	...
	*maxpow = NULL;
	strCommand.Format("scan %d maxidx %d 0.01\r", chan + 1, npoint);
	...
	if (m_strErrorLog.Find("Err:") >=0)
	{
		m_strErrorLog = ("÷∏¡Ó∑µªÿ¥ÌŒÛ£°");
		return FALSE;
	}
...
BOOL CPDBoxForMCS::NewSendPDMon(int chan, int npoint)
{
	...
	ZeroMemory(chRead, sizeof(char) * 50);
	strCommand.Format("scan %d adcmax %d\r", chan + 1, npoint);
	//strCommand.Format("scan 0 adcmax 1024\r");//0 ALL SCAN
	memcpy(chCommand, strCommand, strCommand.GetLength());
...
BOOL CPDBoxForMCS::GetAllPower(double *dbreadpow)
{
	CString strCommand, strValue, strGetValue;
	char    chCommand[50];
	char    chRead[2048];
	...
	if (!ReadBuffer(chRead, 2048,NULL))
	{
		m_strErrorLog.Format("%s", &chRead);
		if (m_strErrorLog.Find("OK", 0) == -1)
		{
