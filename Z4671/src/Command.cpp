...
BOOL CCommand::SetCommand(WORD iCommandAddr, CString strTxData)
{
	...
	sprintf(pchWrite, "seti2c %s %02x %s\r\n", m_I2CAddress, iCommandAddr, strTxData);
	...
	BYTE bIndex = 0;
	for( bIndex = 0; bIndex < 200; bIndex ++)
	{
...
BOOL CCommand::UpgradeCommand(WORD iCommandAddr, BYTE byTxData)
{
	...
	ZeroMemory(pchWrite, 120);
	sprintf(pchWrite, "seti2c %s %02x %02x\r\n", m_I2CAddress, iCommandAddr, byTxData);
...
BOOL CCommand::ByteSetCommand(WORD iCommandAddr, CString strTxData)
{
	...
	ZeroMemory(pchWrite, 120);
	sprintf(pchWrite, "SBI2C %s %02x %s\r\n", m_I2CAddress, iCommandAddr, strTxData);
// 	sprintf(pchWrite, "seti2c c0 %02x %s\r\n", iCommandAddr, strTxData);
...
BOOL CCommand::GetCommand(WORD iCommandAddr, WORD wRxDataLen, CString *strTxData)
{
	...
	ZeroMemory(pchWrite, 120);
	sprintf(pchWrite, "seti2c %s %02x\r\n", m_I2CAddress, iCommandAddr);
	...
	ZeroMemory(pchWrite, 120);
	sprintf(pchWrite, "geti2c %s %02x\r\n", m_I2CAddress, wRxDataLen);
	strRead = pchWrite;
...
//	strTemp = "Seti2c C0 d9 01\r\n";
	strTemp.Format("Seti2c %s d9 01\r\n", m_I2CAddress);
...
//	strTemp.Format("seti2c c0 %x\r\n", bAddress);
	strTemp.Format("seti2c %s %x\r\n", m_I2CAddress, bAddress);
