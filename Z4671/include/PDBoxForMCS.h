...
public:
	...
	BOOL GetPDPower(double *dbreadpow);
	BOOL NewSendPDMon(int chan, int npoint);
	BOOL NewGetPDTriCout(int chan);
	BOOL NewGetMaxPdCout(int chan, int npoint, int *maxid, double *maxpow,double checkdb=0.01);
	bool NewGetMaxPdCout(int chan, int npoint, double checkdb, WORD *maxindex);
