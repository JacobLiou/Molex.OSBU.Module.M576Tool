...
flowchart TD
  ...
  S1 --> G1["当次段 Strict Ok?"]
  G1 -->|No| S2["k=2 右 C+200 采集"]
  G1 -->|Yes| M1["merge 当前段"]
  M1 -->|Ok| C
  M1 -->|Fail| S2
  S2 --> G2["当次段 Strict Ok?\n或 k=2 对称三段齐全"]
  G2 -->|No| E1["k=3/k=4 explore"]
  G2 -->|Yes| M2["merge Relaxed 单峰"]
  M2 -->|Ok| C
...
| 拼接位移步进 | **整窗宽 2R=400**（`M576_PEAK1D_STITCH_TILE_DAC`）；k=1/k=2 对称 ±2R；k=3/k=4 explore ±4R |
| 拼接方向 | k=1 左、k=2 右（对称采集）；k=3/k=4 扩展摸索 |
| 拼接次数上限 | **对称 2 次 + 扩展 2 次**（不含中心段）；**k=1..4 每次 append 后**：当次段 **Strict 单段寻峰 Ok** 才 merge（**k=2 且 k=0,1,2 对称三段齐全豁免**）；merge Ok 即停；**统一 fineRefine@64** |
| 合成寻峰 | **统一** `FindPeakOnPow(merged, FineRefineRelaxed)` 单峰（已移除 mesa/dual_knee） |
| 业务早退 | `PmRangeMismatch` 仍立即失败（INV-16） |

**拼接位移表**（`anchorBase` = 首次@200 **扫频中心** `floor(col0 + halfRange)`）：

| 拼接次序 k | 方向 | `movingBase` | 相对锚点 C | DAC 区间（半窗 R） |
|-----------|------|--------------|------------|-------------------|
| 0 | 中心 | C | 0 | [C−R, C+R] |
| 1 | 左 | C − 2R | −2R | [C−3R, C−R] |
| 2 | 右 | C + 2R | +2R | [C+R, C+3R] |
| 3 | explore 左 | C − 4R | −4R | [C−5R, C−3R] |
| 4 | explore 右 | C + 4R | +4R | [C+3R, C+5R] |

公式：`tier=(k+1)/2`，`offset=tier*2*R`；奇 k → `C−offset`，偶 k → `C+offset`。`C = StitchAnchorCenterFromCoarseSweep(col0, R)`。merge 同 DAC **保留较小 stitchK**（无重叠区平均）。explore（k≥3）PM 超挡位：同向 blocked，仍试反向；双侧 explore PM 失败后 merge 或 FATAL。
...
| `MergeRecal1DSweepSegments(segments, dacStep)` | 按 DAC 排序合并；同 DAC 保留 **较小 stitchK**（无重叠平均） |
