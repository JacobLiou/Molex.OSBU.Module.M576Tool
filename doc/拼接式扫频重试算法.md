...
| 拼接次数上限 | **对称 2 次 + 扩展 2 次**（不含中心段）；**k=1..4 每次 append 后**：当次段 **Strict 单段寻峰 Ok** 才 merge（**k=2 且 k=0,1,2 合成 mesa 豁免**）；merge Ok 即停；**dual-knee mesa 直接成功**（不发 fineRefine） |
