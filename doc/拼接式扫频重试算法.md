...
flowchart TD
  ...
  C0 --> S1["k=1 左 anchor-200 采集（不 merge）"]
  ...
  DK -->|Ok| C
  RL -->|Ok| C
  DK -->|Fail| E1["k=3 explore anchor-400 Relaxed"]
  RL -->|Fail| E1
  E1 -->|Ok| C
  E1 -->|Fail| E2["k=4 explore anchor+400 Relaxed"]
  E2 -->|Ok| C
  E2 -->|Fail| FAIL["整轴失败 + FATAL stitch_k4"]
```
...
| 拼接次数上限 | **对称 2 次 + 扩展 2 次**（不含中心段）；k=1 仅采集，k=2 起 merge |
| 合成寻峰 | 平台峰 → 双拐点；非平台/扩展 → FineRefineRelaxed |
| 业务早退 | `PmRangeMismatch` 仍立即失败（INV-16） |

**拼接位移表**（`anchorBase` = 首次@200 **固件返回 col0**，非 9999 占位 movingBase）：

| 拼接次序 k | 方向 | `movingBase` | 相对锚点位移 | merge |
|-----------|------|--------------|-------------|-------|
| 0 | 中心 | 首次@200 col0 | 0 | 否 |
| 1 | 左 | `col0 - 200` | −200 | 否 |
| 2 | 右 | `col0 + 200` | +200 | **是**（中心+左+右） |
| 3 | explore 左 | `col0 - 400` | −400 | 是（Relaxed） |
| 4 | explore 右 | `col0 + 400` | +400 | 是（Relaxed） |

公式：k=1 → `anchorBase - UNIT`；k=2 → `anchorBase + UNIT`；k=3 → `anchorBase - 2*UNIT`；k=4 → `anchorBase + 2*UNIT`。`anchorBase = floor(col0)`。k=2 首次 merge：平台峰用 **FindPlateauDualKneePeak1D**，否则 **FineRefineRelaxed**；k=3/k=4 扩展仅 Relaxed。
