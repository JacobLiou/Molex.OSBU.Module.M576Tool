---
name: 拼接式扫频重试
overview: 用确定性的三阶段状态机（64 → 200 → 拼接/精扫）替换现有 INV-10/11/12 启发式 planner：粗扫@200失败后左右交替平移拼接（最多3次，位移200/400/600），合成曲线一旦找到峰即停，再发 offset=m_dacRange/2 精扫；Y/X 与 cross 重炉共用同一引擎。
todos:
  - id: pipeline-core
    content: 新增拼接合并 + Resample + Recal1DSweepPipelineState 状态机（Peak1DSweepRecenter 或新文件）及常量
    status: in_progress
  - id: dlg-integrate
    content: 改造 RunRecal1DSweepWithPeakRecenterRetry 为 phase 驱动；精扫 offset=uiFineRange/2；整轴失败写 FATAL 日志
    status: pending
  - id: cross-resweep
...
flowchart TD
  ...
  B -->|寻峰 Ok| C["Phase3: 以粗峰 DAC 为中心 offset=uiFineRange/2 精扫 Relaxed"]
  C --> OKfine[成功]
  B -->|寻峰 Fail| D["Phase4: 以首次@200 为锚点交替拼接"]
  D --> S1["拼接1: 左移 anchor-200，合并寻峰"]
  S1 -->|已有峰| C
  S1 -->|无峰| S2["拼接2: 右移 anchor+400，合并寻峰"]
  S2 -->|已有峰| C
  S2 -->|无峰| S3["拼接3: 左移 anchor-600，合并寻峰"]
  S3 -->|已有峰| C
  S3 -->|仍 Fail| FAIL["整轴失败 + FATAL 日志"]
```
...
| 精扫半窗 | `uiFineRange / 2`（UI=64 → offset=32） |
| 拼接位移步进 | 第 k 次拼接相对锚点 **k × 200 DAC**（k=1,2,3） |
| 拼接方向 | **左右交替**：k=1 左、k=2 右、k=3 左 |
| 拼接次数上限 | **3 次**（不含首次@200）；**合成已有峰则立即停**，不再继续拼接 |
| 业务早退 | `PmRangeMismatch` 仍立即失败（INV-16） |

**拼接位移表**（`anchorBase` = 首次@200 的 `movingBase`）：

| 拼接次序 k | 方向 | `movingBase` | 相对锚点位移 |
|-----------|------|--------------|-------------|
| 1 | 左 | `anchorBase - 200` | −200 |
| 2 | 右 | `anchorBase + 400` | +400 |
| 3 | 左 | `anchorBase - 600` | −600 |

公式：`k` 为奇 → 左（`anchorBase - k*200`）；`k` 为偶 → 右（`anchorBase + k*200`）。每次新扫 append 到 `segments[]` 后，对**当前全部段**合并并寻峰；**一旦 Ok 即进入精扫，不执行后续拼接**。
...
| 精扫 `uiFineRange` 全宽 | 精扫 **半宽** `uiFineRange/2` |
...
struct Recal1DSweepPipelineState {
  int uiFineRange;
  int fineHalfRange;   // uiFineRange/2
  int coarseRange;     // 200
  int stitchShift;     // 200
  int anchorBase;      // 首次@200 的 movingBase
...
1. 保留首次 **Phase2 @200** 失败曲线 → `segments[0]`，`anchorBase = movingBase`  
...
3. FineRefine：`movingBase` 对准合成峰 DAC（`PeakBaseFromCoarseHint` / `round(peakDac)`），`halfRange = fineHalfRange`，`FineRefineRelaxed`

辅助纯函数建议：`StitchShiftDacFromAnchor(k)` → 有符号位移（k=1→−200, k=2→+400, k=3→−600）；`IsStitchLeft(k)` → k 奇为左。
...
- 日志新标签：`coarse200:`、`stitch left:`、`stitch right:`、`fine half:`
...
| 精扫（fine half）仍失败 | 已有粗/合成峰 DAC 但 Relaxed 仍拒 |
...
struct PeakPipelineFailureReport {
  ...
  const char* failedPhase;       // "fine64"|"coarse200"|"stitch_k"|"fineHalf"
...
| 拼接1 fail → 拼接2(右400) Ok | 位移为 anchor+400；早停 |
| 拼接1..3 全 fail | 整轴失败；`M576LogPeakPipelineFatal`；`failedPhase=stitch_k3` |
| `StitchShiftDacFromAnchor(2)==+400` | 单元测试位移公式 |
