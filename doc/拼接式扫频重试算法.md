...
**拼接位移表**（`anchorBase` = 首次@200 **固件返回 col0**，非 9999 占位 movingBase）：
...
公式：`k` 为奇 → 左（`anchorBase - k*200`）；`k` 为偶 → 右（`anchorBase + k*200`）。`anchorBase = floor(col0)`。每次新扫 append 到 `segments[]` 后，对**当前全部段**合并并寻峰；**一旦 Ok 即进入精扫，不执行后续拼接**。
