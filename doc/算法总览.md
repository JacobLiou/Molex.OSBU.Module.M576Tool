...
> 本文档是上位机**寻峰 / 扫频 / 拼接**相关算法的入口说明，面向维护者与产线问题定位。  
> 详细条文见工程内 `M576Calibrator/Docs/INVARIANTS.md`（INV-10..19）、`PeakFinder.md`；拼接状态机见 [`拼接式扫频重试算法.md`](拼接式扫频重试算法.md)。  
> **2026-07-13**：已移除 mesa/dual_knee；三段 merge 统一 `FineRefineRelaxed` 单峰。
...
| 层 | 路径 | 职责 |
|----|------|------|
| 寻峰核心 | `M576CalibratorApp/PeakFinder2D.cpp` | 一维/二维寻峰、三阶拟合（已移除 mesa/dual_knee） |
...
flowchart TD
  ...
  P2["Phase2 严格\n range=200"]
  P3["Phase3 缩小到64精扫"]
  ST["Phase4 左移200扫描 右移200扫描 Stitch 拼接"]
  P1 -->|Ok| OK64["成功返回"]
  P1 -->|Fail| P2
  P2 -->|Ok| P3
  P2 -->|Fail| ST
...
| Stitch | 200 | 扫频 Strict；k=2 对称三段齐全可豁免 Strict 后 merge | merge **Relaxed 单峰** → **FineRefine** |
| FineRefine | `m_dacRange` | `FineRefineRelaxed` | 成功写 LUT 前最后一轮 |

粗扫定位用 Strict；精扫用 Relaxed（三阶失败可 argmax 回退，INV-11）。

### 3.2 拼接位移（anchor = 首次 @200 扫频中心）

`anchorBase = floor(col0 + halfRange)`（几何中心；`base=9999` 占位时由固件 `col0` 推算）。

| stitchK | 方向 | movingBase | DAC 窗 [C±200] |
|---------|------|------------|----------------|
| 0 | 中心 | 首次 @200 | [C?200, C+200] |
| 1 | 左 | C ? 200 | [C?400, C] |
| 2 | 右 | C + 200 | [C, C+400] |
| 3 | explore 左 | C ? 400 | [C?600, C?400] |
| 4 | explore 右 | C + 400 | [C+200, C+600] |

**每次 stitch append 后立即 merge**；寻峰 Ok 则早停进入 FineRefine，跳过剩余 stitch。
...
| `FineRefineRelaxed` | 精扫、拼接 merge、扩展扫 merge | 跳过部分平坦门控；三阶失败可 **argmax 回退** |

### 4.2 常规尖峰（单峰 / 高斯型）

- `ParabolaVertexMax1D`：离群剔除后三阶拟合求 `t*`
- `FindUnimodalPeak1DIndex`：校验局部单峰性
- 用于：fine64 / coarse200 单段、**全部**拼接 merge（含对称三段）、**k≥3 扩展扫**

---

## 5. 合成寻峰（`FindPeakDacOnMerged`）

```
merge 全部 segments
  → span 门限（MinProminenceDb）
  → FindPeakOnPow(merged, FineRefineRelaxed)
  → DacAtMergedSampleIndex 定粗位
  → fineRefine@64
```

**k=2 对称三段**（k0+k1+k2 齐全）时豁免当次 Strict，直接 merge；日志：`symmetric trio merge relaxed`、`merged unimodal relaxed t*=…`。
...
FATAL 输出：`[FATAL][peak-pipeline]` + `m576_fatal.log`，含 `anchor`、`stitch_k`、`t*`、`merged_span`。
...
- 64 fail → 200 ok → fine
- 拼接：k=2 对称三段 → Relaxed 单峰；step97 类 bell 不再误判 mesa
...
5. 拼接 merge：**统一 Relaxed 单峰**；未改固件前勿删拼接 / 扩展扫路径。

---

*文档版本：2026-07-13 — 已移除 mesa/dual_knee，拼接 merge 统一 FineRefineRelaxed。*
