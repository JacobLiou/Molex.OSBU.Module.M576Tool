...
| ID | ������ |
|----|--------|
| **INV-10** | **拼接式扫频 pipeline**（`Peak1DSweepPipeline` / `RunRecal1DSweepWithPeakRecenterRetry`）：**Phase1** `offset=uiFineRange`（UI `m_dacRange`，通常 64）`Strict` — Ok 则**直接返回**，不再精扫；Fail → **Phase2** `offset=M576_MAX_DAC_RANGE`（200）`Strict` — Ok → **Phase3 精扫** `offset=uiFineRange`（同 `m_dacRange`）`FineRefineRelaxed`；Fail → **Phase4 拼接**：以首次@200 **固件返回 col0**（首格 DAC）为 `anchorBase`（`base=9999` 时不可用 movingBase），append 中心段（stitchK=0）后 **强制** k=1 左 `anchor−200`、k=2 右 `anchor+200` 各扫一次（k=1 不 merge）；三段齐全后 DAC 坐标合并：**平台峰**（`IsMergedMesaProfile`）→ `FindPlateauDualKneePeak1D`；**非平台峰** → `FineRefineRelaxed`；对称三段仍无峰 → k=3/k=4 扩展扫 `anchor±400`（**仅 Relaxed**，禁止双拐点）→ 精扫全宽；拼接 k=1..4 耗尽仍无峰 → **整轴 FATAL** `stitch_k4`（见 INV-15）。硬顶扫频 `M576_PEAK1D_PIPELINE_MAX_SWEEPS`（11）。`PlanNextRecal1DSweepAttempt` 等旧 planner **仅 CrossPeakTest 遗留自测**，产线路径不再调用。**平坦门限**统一为 INI `MinProminenceDb`（默认 0.3 dB）：离群剔除后 `useOk` 点 `max−min`（`Peak1DMinFlatSpanRaw`）；已移除 2.5 dB / `relFlat` 第二套判据。 |
