...
| ID | ������ |
|----|--------|
| **INV-10** | **拼接式扫频 pipeline**（`Peak1DSweepPipeline` / `RunRecal1DSweepWithPeakRecenterRetry`）：**Phase1** `offset=uiFineRange`（UI `m_dacRange`，通常 64）`Strict` — Ok 则**直接返回**，不再精扫；Fail → **Phase2** `offset=M576_MAX_DAC_RANGE`（200）`Strict` — Ok → **Phase3 精扫** `offset=uiFineRange`（同 `m_dacRange`）`FineRefineRelaxed`；**Phase2 Ok 且 Phase3 失败且 segments 空** → **回退 Stitch**（append 缓存的 coarse@200 为 k=0，继续 k=1..4），**非 terminal**；Phase2 Fail → **Phase4 拼接**：以首次@200 **扫频几何中心** `anchorBase = floor(col0 + halfRange)` 为基准（`base=9999` 占位时由固件 `col0` 推算），append 中心段（stitchK=0）后 k=1..4 依次左/右/扩展扫；**平移步长=整窗宽 `2*halfRange`（`M576_PEAK1D_STITCH_TILE_DAC`）**，相邻段 DAC **首尾相接、无半窗重叠**；`MergeRecal1DSweepSegments` 同 DAC 保留较小 `stitchK`（接缝不平均）；**每次 stitch append（k=1..4）后**：**当次段须 Strict 单段寻峰 Ok** 才尝试 merge（**例外**：k=2 且 k=0,1,2 合成 `IsMergedMesaProfile` 时豁免，因左右 flank 单段必 Strict fail）；merge 当前全部 segments 寻峰 Ok 则**早停**（跳过剩余 stitch）；merge：**平台峰**且 k=0,1,2 齐全 → `FindPlateauDualKneePeak1D` 定粗位后**仍须 fineRefine@64**（与 coarse@200 Ok 一致，不再 skip）；merge DAC 用 `DacAtMergedSampleIndex(mergeCol0, t*, dacStep)`；dual-knee 前 `ValidateMergedYPowerAtPeak`；**非平台 / 仅两段 / 含 explore** → `FineRefineRelaxed` 后 fineRefine；k=1..4 均 merge 失败 → **整轴 FATAL** `stitch_k4`（见 INV-15）。**explore（k>=3）PM 超挡位**（INV-16）：`HandleStitchExplorePmRangeReject` — 不 append 该段、同向 blocked、仍试反向 explore；双侧 explore 均 PM 失败后 merge 或 `stitch_pm_explore` FATAL；**k=1/k=2 PM 仍立即 FATAL**。硬顶扫频 `M576_PEAK1D_PIPELINE_MAX_SWEEPS`（11）。`PlanNextRecal1DSweepAttempt` 等旧 planner **仅 CrossPeakTest 遗留自测**，产线路径不再调用。**平坦门限**统一为 INI `MinProminenceDb`（默认 0.3 dB）：离群剔除后 `useOk` 点 `max−min`（`Peak1DMinFlatSpanRaw`）；已移除 2.5 dB / `relFlat` 第二套判据。**mesa cross**（仅单测/遗留）：`PeakCrossFromMesaMergedYAndFineX` 须 `ValidateMergedYPowerAtPeak` 通过后才认 Y。 |
