...
2. **平坦门限（Strict）**：离群剔除后 useOk 点 `max?min >= MinProminenceDb`（默认 0.3 dB，`Peak1DMinFlatSpanRaw()`；INI `MinProminenceDb` 可覆盖），未达则 `ParabolaNotDownward`。  
...
flowchart TD
  ...
  B -->|Ok| C["Phase3: ??? offset=uiFineRange/2 Relaxed"]
  C --> OKfine[???]
  B -->|Fail| D["Phase4: ?????@200 ??????????"]
  D --> S1["k=1 ?? anchor-200 ??????"]
  S1 -->|????| C
  S1 -->|???| S2["k=2 ?? anchor+400"]
  S2 -->|????| C
  S2 -->|???| S3["k=3 ?? anchor-600"]
  S3 -->|????| C
  S3 -->|?? Fail| FAIL["FATAL peak-pipeline"]
```
...
???????????????? fine half??`offset=m_dacRange/2`??Relaxed????
...
|------|----------|------|
| ???? pipeline | 6 | 64+200+????3???+??? |
| ???? | `M576_PEAK1D_PIPELINE_MAX_SWEEPS`??9?? | ??????? |
