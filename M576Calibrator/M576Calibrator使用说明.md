...
- **1310 低温槽**：波长选 **1310 nm** 跑路径后，写 BIN 时将会话定标结果 **仅合并到低温槽（`IDX_TEMP_LOW` / `stCalibDAC[0]`）**，其余温度点从备份 BIN 保留。
- **1550 常温+高温槽**：波长选 **1550 nm** 跑路径后，寻峰写入 **常温槽**；写 BIN 时合并常温并 **复制到高温槽**，两区温度元数据均为 **30°C**（MCS `wTemperaturePoint`=300，1×64 `sTemperature`=3000）。
- **会话策略**：Run Path 开始时根据波长锁定策略；Write BIN 使用 **会话波长**（若界面波长已改会打警告日志）。
