...
- **PD（A+C）**：`RECAL 2`，目标索引 **1～4**（依次为 2#1x64 Stage1、2#1x64 Stage2、1#MCS、2#MCS），两段光路为 `[2#1x64][1#MCS]` 或 `[2#1x64][2#MCS]`；硬件上 2×8 开关接 `Ext1.Laser`、连接 `OPM`（以 Z4744 说明为准）。  
- 两种模式共用同一套 **RECAL 0** 扫参；仅 **RECAL 1 / RECAL 2** 及 CSV 列不同。
...
```text
target_index, p2b, p2c, p3b, p3c
```

- **target_index**：**1～4**（1/2 = 2#1x64 Stage1/2；3 = 1#MCS；4 = 2#MCS）。  
- **p2b, p2c**：2#1x64 段，**p2b** 为 Stage（1 或 2），**p2c** 为通道 1～64。  
- **p3b, p3c**：MCS 段，**p3b** 为 1# 或 2# MCS（1 或 2），**p3c** 为 1～18。  
- **路由**：2#1x64 **ch 1～32** 必须对应 **1# MCS（p3b=1）**；**ch 33～64** 必须对应 **2# MCS（p3b=2）**。  
