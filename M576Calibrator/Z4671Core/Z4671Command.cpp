...
BOOL Z4671Command::GetModulePara()
{
	...
	{
		m_strLogInfo.Format("CmdSendExchange函数执行错误");
		return FALSE;
	}
	
	if(!WriteBuffer((char*)pbySendData,nCmdLength))
	{
		m_strLogInfo.Format("发送ModulePara指令错误");
		return FALSE;
	}
	Sleep(50);
	BYTE  byGetData[2048];
	...
	{
		m_strLogInfo.Format("接收ModulePara指令错误");
		return FALSE;
	}
	...
	{
		m_strLogInfo.Format("CmdReadExchange函数执行错误(%s)",strReadData);
		return FALSE;
	}
	if (pbyNewData[1]!=0X00)
	{
		m_strLogInfo.Format("返回第2个字节码:%02X(%s)",pbyNewData[1],strReadData);
		return FALSE;
	}
	if (pbyNewData[2]!=byData[2])
	{
		m_strLogInfo.Format("返回第3个字节码:%02X(%s)",pbyNewData[2],strReadData);
		return FALSE;
	}
	if ((pbyNewData[3]*256+pbyNewData[4])!=646) 
	{
		m_strLogInfo.Format("返回第4个字节码:%02X(%s)",pbyNewData[4],strReadData);
		return FALSE;
	}
	if (pbyNewData[5]!=0XB2) 
	{
		m_strLogInfo.Format("返回第6个字节码:%02X(%s)",pbyNewData[5],strReadData);
		return FALSE;
	}
	int nIndex = 0;
...
BOOL Z4671Command::CmdSendExchange(PBYTE pbyte, WORD wLength, PBYTE *pReBuff, PWORD pwReLength)
{
	...
	{
		//LogInfo("指令数量超标",(BOOL)FALSE,COLOR_RED);
		return FALSE;
	}
...
//	DWORD dwLength;
	...
	*/
	if(!WriteBuffer("\r\n",1))
	{
		m_strLogInfo = "EDFA_SRESET测试中发送第一条指令错误";
		return FALSE;			
	}
	Sleep(100);
...
BOOL Z4671Command::EDFA_SRESETTWOCMD()
{
	...
	{
		m_strLogInfo = "EDFA_SRESET测试中发送第二条指令错误";
		return FALSE;			
	}
	Sleep(50);
	ZeroMemory(bBuffer,600);
	...
	for(int i=0;i<29;i++)
	{
		...
		{
			m_strLogInfo = "EDFA_SRESET测试中接收第二条指令返回内容与实际配置不相符";
			return FALSE;			   
		} 
	}
...
BOOL Z4671Command::IDX_HARD_RST()
{
	...
	if(!WriteBuffer((char*)bBuffer,dwLength))
	{
		m_strLogInfo = "IDX_HARD_RST测试中发送指令错误";
		return FALSE;			
	}
	Sleep(50);
	ZeroMemory(bBuffer,600);
	...
	for(int i=0;i<6;i++)
	{
		if(bBuffer[i+1] != bResult[i])
		{
			m_strLogInfo = "IDX_HARD_RSTT测试中接收指令返回内容与实际配置不相符";
			return FALSE;			   
		} 
	}
...
BOOL Z4671Command::IDX_SOFT_RST()
{
	...
	if(!WriteBuffer((char*)bBuffer,dwLength))
	{
		m_strLogInfo = "IDX_SOFT_RST测试中发送指令错误";
		return FALSE;			
	}
	Sleep(50);
	ZeroMemory(bBuffer,600);
	...
	for(int i=0;i<29;i++)
	{
		if(bBuffer[i+1] != bResult[i])
		{
			m_strLogInfo = "IDX_SOFT_RSTT测试中接收指令返回内容与实际配置不相符";
			return FALSE;			   
		} 
	}
...
BOOL Z4671Command::GetMCSTemp(double *dblTemp)
{
	...
	{
		//m_strLogInfo.Format("状态码错误：0X%02X",pbyNewGetData[1]);
		return FALSE;
	}
	if (pbyNewGetData[2]!=byData[2])
	{
		//m_strLogInfo.Format("序号错误：0X%02X",pbyNewGetData[2]);
		return FALSE;
	}
	SHORT sTemp;
	BYTE* pch = (BYTE*)&sTemp;
...
BOOL Z4671Command::GetEDFATemp(double *dblTemp)
{
	...
	if (pbyNewData[1]!=0)
	{
		//m_strLogInfo.Format("状态码错误：0X%02X",pbyNewData[1]);
		return FALSE;
	}
	if (pbyNewData[2]!=byData[2])
	{
		//m_strLogInfo.Format("序号错误：0X%02X",pbyNewData[2]);
		return FALSE;
	}
